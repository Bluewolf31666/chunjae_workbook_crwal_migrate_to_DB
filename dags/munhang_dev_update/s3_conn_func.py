...
######### S3  UPLOAD PNG #########  
def upload_s3_png(s3, bucket, path_list, munhang_list, bucket_object):
    try:
        filename = os.path.basename(path_list)
        object_path  = bucket_object.format(munhang_list) + filename
...
######### S3 UPLOAD SVG #########    
def upload_s3_svg(s3, bucket, path_list, munhang_list, bucket_object):
    try:
        filename = os.path.basename(path_list)
        object_path  = bucket_object.format(munhang_list) + filename
        svg_path = bucket_object.format(munhang_list) + filename
        s3.upload_file(path_list, bucket, object_path, ExtraArgs={'ContentType': 'image/svg+xml'})
